...
\begin{tikzpicture}[font=\sffamily, >=Stealth]
  ...
  \draw[line width=2pt, draw=gray!40] (0.2, 13.5) -- (12.2, 13.5);
  ...
  \fill[fairmidred!20] (0.2, 12.3) rectangle (12.2, 13.1);
  \draw[thick, draw=fairmidred] (0.2, 13.1) -- (12.2, 13.1);
  \draw[thick, draw=fairmidred] (0.2, 12.3) -- (12.2, 12.3);
  ...
  \node[fill=fairblue, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (11.0, 12.7) {espB (-6.0)};

  % Layer 2: Periplasmic Space & Arabinogalactan-Peptidoglycan (AG-PG) Mesh
  \fill[gray!6] (0.2, 10.9) rectangle (12.2, 12.3);
  \draw[dashed, draw=gray!40] (0.2, 11.6) -- (12.2, 11.6);
  ...
  \fill[fairblue!15] (0.2, 9.1) rectangle (12.2, 10.9);
  \draw[thick, draw=fairblue] (0.2, 10.9) -- (12.2, 10.9);
  \draw[thick, draw=fairblue] (0.2, 9.1) -- (12.2, 9.1);
  ...
  \draw[->, thick, draw=fairred] (4.65, 9.2) -- (4.65, 10.4) node[above, font=\tiny\bfseries] {O$_2 \to$ H$_2$O};
  ...
  % Nitrate / Alternate Transporters
  \draw[fill=fairblue, draw=fairblue!80!black, rounded corners=2pt] (11.0, 9.3) rectangle (12.0, 10.3);
  \node[font=\tiny\bfseries, text=white, align=center] at (11.5, 9.8) {fadD7\\(-5.3)};

  % Layer 4: Cytoplasm (Metabolic Cascades)
  \fill[fairlight] (0.2, 4.8) rectangle (12.2, 9.1);
  \node[anchor=west, font=\tiny\bfseries, text=darkgray] at (0.3, 8.8) {Cytoplasm: Biochemical Cascades \& Metabolic Shunts};

  % Cascade A: Nitrogen & Polyamine Shunt (Extreme Activation)
  \draw[thick, draw=fairred!40, fill=white] (0.4, 5.0) rectangle (4.1, 8.5);
  ...
  \node[fill=fairred, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (2.2, 5.7) {ilvB / als (+6.1)};
  \node[font=\tiny, anchor=west] at (0.5, 5.3) {Pyruvate $\to$ 2-Acetolactate};

  % Cascade B: FAS-II Mycolic Acid Synthesis
  \draw[thick, draw=fairmidred!40, fill=white] (4.4, 5.0) rectangle (8.0, 8.5);
  \node[font=\tiny\bfseries, text=fairred!90!black, anchor=west] at (4.5, 8.2) {2. Mycolic Acid FAS-II Spiral};
  \node[font=\tiny, anchor=west] at (4.5, 7.7) {Acetyl-CoA $\xrightarrow{\textbf{accD}}$ Malonyl-CoA};
  \draw[->, thick, draw=fairmidred] (6.2, 7.5) -- (6.2, 6.7);
  \node[fill=fairred, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (6.2, 7.1) {kasA (+1.8)};
  \node[font=\tiny, anchor=west] at (4.5, 6.4) {$\beta$-ketoacyl-AcpM $\xrightarrow{\textbf{inhA}}$ Acyl-AcpM};
  \node[font=\tiny, anchor=west] at (4.5, 5.7) {Meromycolate C70--C90 synthesis};
  \node[font=\tiny\bfseries, text=fairred!80!black, anchor=west] at (4.5, 5.3) {Cell Envelope Fortification};

  % Cascade C: Central Carbon & Redox Systems
  \draw[thick, draw=fairblue!40, fill=white] (8.3, 5.0) rectangle (12.0, 8.5);
  \node[font=\tiny\bfseries, text=fairblue!90!black, anchor=west] at (8.4, 8.2) {3. Redox \& Cofactor Systems};
  \node[fill=fairred, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (9.4, 7.7) {adhP (+7.0)};
  \node[font=\tiny, anchor=west] at (10.1, 7.7) {Alcohol redox};
  \node[fill=fairred, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (9.4, 7.0) {ribD (+6.2)};
  \node[font=\tiny, anchor=west] at (10.1, 7.0) {Riboflavin / FAD};
  \node[fill=fairred, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (9.4, 6.3) {F420 (+6.0)};
  \node[font=\tiny, anchor=west] at (10.1, 6.3) {LLM oxidoreductase};
  \node[fill=fairblue, text=white, font=\tiny\bfseries, rounded corners=2pt, inner sep=2pt] at (9.4, 5.6) {icl1 (-0.7)};
  \node[font=\tiny, anchor=west] at (10.1, 5.6) {Glyoxylate bypass};

  % Panel b: Subsystem Perturbation Index (SPI) Bar Plot
  ...
  \draw[->, thick] (4.8, 0.6) -- (12.0, 0.6) node[midway, below=5pt, font=\scriptsize\bfseries] {Subsystem Perturbation Index (SPI)};
  \draw[thick] (4.8, 0.6) -- (4.8, 4.0);

  \foreach \x/\lbl in {4.8/0, 6.2/5, 7.6/10, 9.0/15, 10.4/20, 11.8/25} {
    \draw (\x, 0.5) -- (\x, 0.6) node[below=2pt, font=\tiny] {\lbl};
    \draw[dashed, draw=gray!20] (\x, 0.6) -- (\x, 4.0);
  }

  % Bars (Y positions: 3.6, 3.15, 2.7, 2.25, 1.8, 1.35, 0.9)
  % 1. Nitrogen Shunts & Polyamines (SPI = 24.52) -> Deep Red
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 3.6) {Nitrogen Shunts \& Polyamines};
  \fill[fairred] (4.8, 3.4) rectangle (11.66, 3.8);
  \node[anchor=west, font=\tiny\bfseries, text=fairred] at (11.75, 3.6) {24.5 (Active, $+6.6$)};

  % 2. Redox Homeostasis & Cofactors (SPI = 10.62) -> Deep Red
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 3.15) {Redox Homeostasis \& Cofactors};
  \fill[fairred!90!white] (4.8, 2.95) rectangle (7.77, 3.35);
  \node[anchor=west, font=\tiny\bfseries, text=fairred!90!black] at (7.87, 3.15) {10.6 (Active, $+3.5$)};

  % 3. Cellular Respiration & Energy (SPI = 5.27) -> Mid Red
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 2.7) {Cellular Respiration \& Energy};
  \fill[fairmidred] (4.8, 2.5) rectangle (6.27, 2.9);
  \node[anchor=west, font=\tiny\bfseries, text=fairmidred] at (6.37, 2.7) {5.3 (Active, $+2.5$)};

  % 4. Type VII Secretion & Virulence (SPI = 2.87) -> Blue
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 2.25) {Type VII Secretion \& Virulence};
  \fill[fairblue!70!white] (4.8, 2.05) rectangle (5.60, 2.45);
  \node[anchor=west, font=\tiny\bfseries, text=fairblue] at (5.70, 2.25) {2.9 (Repressed, $-0.5$)};

  % 5. GPL Biofilm & Cell Surface (SPI = 2.69) -> Blue
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 1.8) {GPL Biofilm \& Cell Surface};
  \fill[fairblue!60!white] (4.8, 1.6) rectangle (5.55, 2.0);
  \node[anchor=west, font=\tiny\bfseries, text=fairblue] at (5.65, 1.8) {2.7 (Repressed, $-0.9$)};

  % 6. Mycolic Acid & FAS-II Envelope (SPI = 1.24) -> Light Red
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 1.35) {Mycolic Acid \& FAS-II Envelope};
  \fill[fairmidred!60!white] (4.8, 1.15) rectangle (5.15, 1.55);
  \node[anchor=west, font=\tiny\bfseries, text=fairmidred!80!black] at (5.25, 1.35) {1.2 (Active, $+0.4$)};

  % 7. DosR Hypoxia & Dormancy (SPI = 0.97) -> Light Blue
  \node[anchor=east, font=\scriptsize\bfseries] at (4.6, 0.9) {DosR Hypoxia \& Central Carbon};
  \fill[fairblue!40!white] (4.8, 0.7) rectangle (5.07, 1.1);
  \node[anchor=west, font=\tiny\bfseries, text=gray!80!black] at (5.17, 0.9) {0.97 (Baseline shift)};
\end{tikzpicture}
\end{document}
"""
